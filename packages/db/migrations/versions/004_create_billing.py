...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("case_id", UUID(as_uuid=True), sa.ForeignKey("cases.id", ondelete="RESTRICT"), nullable=False, index=True),
        ...
        sa.Column("rounding_rule", sa.String(10), nullable=False, server_default=sa.text("'6min'")),
        sa.Column("source", sa.String(20), nullable=False, server_default=sa.text("'MANUAL'"), index=True),
    ...
    op.create_table(
        ...
        sa.Column("id", UUID(as_uuid=True), server_default=sa.text("gen_random_uuid()"), primary_key=True),
        sa.Column("tenant_id", UUID(as_uuid=True), sa.ForeignKey("tenants.id", ondelete="RESTRICT"), nullable=False, index=True),
        sa.Column("case_id", UUID(as_uuid=True), sa.ForeignKey("cases.id", ondelete="RESTRICT"), nullable=True, index=True),
        sa.Column("invoice_number", sa.String(50), nullable=False),
        sa.Column("client_contact_id", UUID(as_uuid=True), sa.ForeignKey("contacts.id", ondelete="RESTRICT"), nullable=False, index=True),
        sa.Column("status", sa.String(20), nullable=False, server_default=sa.text("'draft'"), index=True),
        ...
        sa.Column("vat_rate", sa.Numeric(5, 2), nullable=False, server_default=sa.text("21.00")),
        sa.Column("vat_amount_cents", sa.BigInteger, nullable=False, server_default=sa.text("0")),
        sa.Column("total_cents", sa.BigInteger, nullable=False, server_default=sa.text("0")),
        sa.Column("peppol_status", sa.String(20), nullable=False, server_default=sa.text("'none'")),
