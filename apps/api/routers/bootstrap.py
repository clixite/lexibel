...
async def seed_demo_data() -> None:
    ...
    async with async_session_factory() as session:
        async with session.begin():
            ...
            # ── Contacts ──
            contacts_data = [
                (_CONTACT_IDS[0], "natural", "Jean Dupont", "jean.dupont@email.be", "+32470123456", None),
                (_CONTACT_IDS[1], "natural", "Marie Van der Berg", "marie.vanderberg@email.be", "+32471234567", None),
                (_CONTACT_IDS[2], "legal", "SA Immobel", "contact@immobel.be", "+32223456789", "0417.497.106"),
                (_CONTACT_IDS[3], "legal", "SPRL TechWave", "info@techwave.be", "+32224567890", "0123.456.789"),
            ]
            for cid, ctype, name, email, phone, bce in contacts_data:
                session.add(Contact(
                    ...
                    bce_number=bce,
                    language="fr",
                    address={"street": "Rue Example 1", "city": "Bruxelles", "zip": "1000", "country": "BE"},
                ))
            await session.flush()
            ...
            today = date.today()
            cases_data = [
                (_CASE_IDS[0], "2024/001", "Dupont c/ Immobel", "civil", "open", _CONTACT_IDS[0]),
                (_CASE_IDS[1], "2024/002", "Succession Van der Berg", "family", "open", _CONTACT_IDS[1]),
                (_CASE_IDS[2], "2024/003", "TechWave Restructuration", "commercial", "open", _CONTACT_IDS[3]),
                (_CASE_IDS[3], "2024/004", "Martin - Harcelement moral", "social", "pending", _CONTACT_IDS[0]),
            ]
            ...
            # ── Time Entries ──
            time_data = [
                (_TIME_ENTRY_IDS[0], _CASE_IDS[0], "Consultation initiale - analyse du dossier Dupont c/ Immobel", 120, today - timedelta(days=30), "approved", 25000),
                (_TIME_ENTRY_IDS[1], _CASE_IDS[1], "Rédaction conclusions succession Van der Berg", 90, today - timedelta(days=15), "approved", 25000),
                (_TIME_ENTRY_IDS[2], _CASE_IDS[2], "Analyse restructuration TechWave - due diligence", 180, today - timedelta(days=7), "draft", 25000),
                (_TIME_ENTRY_IDS[3], _CASE_IDS[0], "Audience TPI Bruxelles - plaidoirie Dupont", 240, today - timedelta(days=3), "draft", 25000),
            ]
            ...
            # ── Invoices ──
            invoices_data = [
                (_INVOICE_IDS[0], _CASE_IDS[0], "FAC-2024-001", _CONTACT_IDS[0], "paid", today - timedelta(days=60), today - timedelta(days=30), 75000, 15750, 90750),
                (_INVOICE_IDS[1], _CASE_IDS[1], "FAC-2024-002", _CONTACT_IDS[1], "sent", today - timedelta(days=14), today + timedelta(days=16), 56250, 11813, 68063),
                (_INVOICE_IDS[2], _CASE_IDS[2], "FAC-2024-003", _CONTACT_IDS[3], "draft", today, today + timedelta(days=30), 112500, 23625, 136125),
            ]
            for invid, caseid, num, client, st, issue, due, sub, vat, total in invoices_data:
                session.add(Invoice(
                    id=invid,
                    tenant_id=tid,
                    case_id=caseid,
            ...
            # Invoice lines
            lines = [
                (_INVOICE_IDS[0], "Consultation initiale (2h)", Decimal("2.00"), 25000, 50000),
                (_INVOICE_IDS[0], "Rédaction mise en demeure (1h)", Decimal("1.00"), 25000, 25000),
                (_INVOICE_IDS[1], "Analyse successorale (1.5h)", Decimal("1.50"), 25000, 37500),
                (_INVOICE_IDS[1], "Rédaction conclusions (0.75h)", Decimal("0.75"), 25000, 18750),
                (_INVOICE_IDS[2], "Due diligence TechWave (3h)", Decimal("3.00"), 25000, 75000),
                (_INVOICE_IDS[2], "Analyse contrats (1.5h)", Decimal("1.50"), 25000, 37500),
            ]
            ...
            # ── Inbox Items ──
            inbox_data = [
                (_INBOX_IDS[0], "OUTLOOK", {"from": "jean.dupont@email.be", "subject": "Documents pour audience", "body": "Veuillez trouver ci-joint les pièces pour l'audience de vendredi.", "received_at": (datetime.now() - timedelta(hours=2)).isoformat()}),
                (_INBOX_IDS[1], "OUTLOOK", {"from": "marie.vanderberg@email.be", "subject": "Question succession", "body": "Pourriez-vous me confirmer la date du rendez-vous chez le notaire?", "received_at": (datetime.now() - timedelta(hours=5)).isoformat()}),
                (_INBOX_IDS[2], "RINGOVER", {"caller": "+32470123456", "callee": "+32223456789", "duration": 342, "direction": "inbound", "recording_url": None}),
                (_INBOX_IDS[3], "PLAUD", {"caller": "+32471234567", "duration": 1205, "transcription": "Discussion concernant la succession Van der Berg...", "language": "fr"}),
            ]
