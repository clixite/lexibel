...
COURT_PATTERNS = [
    (re.compile(r"\b(Tribunal\s+de\s+premi[èe]re\s+instance\s+de\s+\w+)\b", re.I), "COURT"),
...
LEGAL_REF_PATTERNS = [
    ...
    (re.compile(r"\b(Art\.?\s*\d+(?:\s*(?:bis|ter|quater))?\s*(?:(?:du\s+)?C\.?\s*[A-Z]\.?(?:\s*[A-Z]\.?)?)?)\b", re.I), "LEGAL_REFERENCE"),
    # Code references: Code civil, Code judiciaire
    (re.compile(r"\b(Code\s+(?:civil|judiciaire|pénal|de\s+commerce|des\s+sociétés))\b", re.I), "LEGAL_REFERENCE"),
    ...
    (re.compile(r"\b(M\.?\s*B\.?\s*(?:du\s+)?\d{1,2}[./]\d{1,2}[./]\d{2,4})\b", re.I), "LEGAL_REFERENCE"),
    # EU regulations: Règlement (UE) 2016/679
    (re.compile(r"\b(R[èe]glement\s*\((?:UE|CE)\)\s*\d{4}/\d+)\b", re.I), "LEGAL_REFERENCE"),
...
PERSON_PATTERNS = [
    # Maître / Me
    (re.compile(r"\b(Ma[îi]tre\s+[A-ZÀ-Ü][a-zà-ÿ]+(?:\s+[A-ZÀ-Ü][a-zà-ÿ]+)?)\b"), "PERSON"),
    (re.compile(r"\b(Me\.?\s+[A-ZÀ-Ü][a-zà-ÿ]+(?:\s+[A-ZÀ-Ü][a-zà-ÿ]+)?)\b"), "PERSON"),
    # Monsieur/Madame
    (re.compile(r"\b((?:Monsieur|Madame|M\.|Mme\.?)\s+[A-ZÀ-Ü][a-zà-ÿ]+(?:\s+[A-ZÀ-Ü][a-zà-ÿ]+)?)\b"), "PERSON"),
...
ORG_PATTERNS = [
    ...
    (re.compile(r"\b([A-ZÀ-Ü][\w\s&.-]+(?:SA|S\.A\.|SPRL|S\.P\.R\.L\.|SRL|ASBL|A\.S\.B\.L\.|SCS|SNC|SCRL))\b"), "ORGANIZATION"),
    (re.compile(r"\b([A-ZÀ-Ü][\w\s&.-]+(?:NV|N\.V\.|BV|B\.V\.|BVBA|B\.V\.B\.A\.|VZW|V\.Z\.W\.))\b"), "ORGANIZATION"),
    # SPF / FOD (federal services)
    (re.compile(r"\b(SPF\s+\w+(?:\s+\w+)?)\b"), "ORGANIZATION"),
    (re.compile(r"\b(FOD\s+\w+(?:\s+\w+)?)\b"), "ORGANIZATION"),
    # Barreau / Ordre
    (re.compile(r"\b((?:Barreau|Ordre\s+des\s+avocats)\s+(?:de|du|d.)\s+\w+)\b", re.I), "ORGANIZATION"),
]

# ── Monetary patterns ──

MONETARY_PATTERNS = [
    (re.compile(r"\b(\d{1,3}(?:[.,]\d{3})*(?:[.,]\d{2})?\s*(?:EUR|€|euros?))\b", re.I), "MONETARY_AMOUNT"),
    (re.compile(r"\b((?:EUR|€)\s*\d{1,3}(?:[.,]\d{3})*(?:[.,]\d{2})?)\b", re.I), "MONETARY_AMOUNT"),
]
...
BELGIAN_CITIES = {
    "Bruxelles", "Brussel", "Anvers", "Antwerpen", "Liège", "Luik",
    "Gand", "Gent", "Charleroi", "Namur", "Namen", "Mons", "Bergen",
    "Bruges", "Brugge", "Leuven", "Louvain", "Hasselt", "Arlon",
    "Aarlen", "Tournai", "Doornik", "Wavre", "Waver", "Nivelles",
    "Nijvel", "Eupen", "Verviers", "Courtrai", "Kortrijk", "Mechelen",
    "Malines", "Louvain-la-Neuve",
}

LOCATION_PATTERN = re.compile(
    r"\b(" + "|".join(re.escape(c) for c in sorted(BELGIAN_CITIES, key=len, reverse=True)) + r")\b",
    re.I,
)
...
class NERService:
    ...
    def _deduplicate(self, entities: list[Entity]) -> list[Entity]:
        ...
        sorted_ents = sorted(entities, key=lambda e: (-e.confidence, -(e.end - e.start)))
        kept: list[Entity] = []
