...
class TestDirectConflicts:
    ...
    async def test_detect_direct_conflicts_found(
        ...
        # Execute
        conflicts = await conflict_detector.detect_direct_conflicts(
            sample_contact_id
        )
...
class TestDirectorOverlap:
    ...
    async def test_detect_director_overlap_found(
        ...
        conflicts = await conflict_detector.detect_director_overlap(
            sample_contact_id
        )
...
class TestBusinessPartners:
    ...
    async def test_detect_business_partners_high_stake(
        ...
        conflicts = await conflict_detector.detect_business_partners(
            sample_contact_id
        )
...
class TestGroupConflicts:
    ...
    async def test_detect_group_conflicts_parent(
        ...
        conflicts = await conflict_detector.detect_group_conflicts(
            sample_contact_id
        )
...
class TestCheckAllConflicts:
    ...
    async def test_check_all_conflicts_company(
        ...
        conflict_detector.detect_ownership_conflicts = AsyncMock(return_value=[{
            'conflicting_id': str(uuid4()),
...
class TestConflictScoreCalculation:
    ...
    def test_calculate_score_bounds(self, conflict_detector):
        ...
        test_cases = [
            {'conflict_type': 'direct_adversary'},
            {'conflict_type': 'historical_conflict', 'last_representation': '2000-01-01'},
...
class TestSingleton:
    ...
    async def test_get_conflict_detector_singleton(self):
        ...
        with patch('apps.api.services.sentinel.conflict_detector.get_neo4j_client') as mock_neo4j:
